...
double pi, xleft, xright, yleft, yright, volume, h, dt, gamphi, gampsi, Cahnphi, Cahnpsi, Mphi, Mpsi, alpha, theta, **Wpsi, **Wphi, Qpsi, **Qphi, **dFphi, **dGpsi,
    ...
    **adv_u, **adv_v, **adv_phi, **adv_psi, **fx, **fy, **fxphi, **fyphi, **fxpsi, **fypsi, **worku, **workv, **workp, **nworku, **nworkv,
    Re, We, rho1, rho2, rho, gravity,velocity ;
...
int main()
{
    ...
    gamphi =4 * h / (2.0 * sqrt(2.0) * atanh(0.9));
   gampsi =4 * h / (2.0 * sqrt(2.0) * atanh(0.9));
    ...
    sprintf(bufferpsi, "./datatest1/datapsi.m");
    sprintf(bufferphi, "./datatest1/dataphi.m");
    sprintf(bufferu, "./datatest1/datau.m");
    sprintf(bufferv, "./datatest1/datav.m");
    sprintf(bufferp, "./datatest1/datap.m");
    ...
    mat_copy(nv, v, 1, nx, 0, ny); // v_{n+1}=v_{n}
    ...
    fmasspsi = fopen("./datatest1/masspsi.m", "w");
    fmassphi = fopen("./datatest1/massphi.m", "w");
    ...
    for (it =1; it <= max_it; it++)
    {
        ...
        /*******************NSinitial********************/
        // caculate advection in NS,in CAC,(adv_u,adv_v)=u.gradu,adv_phi=div(phiu),adv_psi=div(psiu)
  ...
  /*      ijloop{
        ...
        // calculate the phase-field: psi
        cahnpsi(psi, opsi, adv_psi, npsi);

        // calculate the phase-field: phi
...
{
    extern double xright, yright, h, gamphi, pi,velocity;
    ...
    ijloop
    {
        ...
        psi[i][j] = 0.2 ;
        // phi[i][j] = 0.3 + 0.001 * (2.0 * rand() / (float)RAND_MAX - 1.0);
        phi[i][j] = tanh(((0.2 ) - sqrt(pow(x - 0.5, 2) / a + pow(y - 0.5, 2) / b)) / (sqrt(2) * gamphi));
        p[i][j] = 0.0;
    }
      i0jloop{
          x = ((double)i - 0.5) * h;
         y = ((double)j - 0.5) * h;
         u[i][j]=velocity*2*(y-0.5);
     }
     ij0loop{
           x = ((double)i - 0.5) * h;
         y = ((double)j - 0.5) * h;
...
void advection_step(double **u, double **v, double **c1, double **c2, double **adv_u, double **adv_v, double **adv_c1, double **adv_c2)
{
    ...
    // conservative in CAC adv_phi= div(phiu)
    ijloop
    {
...
void augmenuv(double **u, double **v)
{
    ...
    for (i = -1; i <= nx + 1; i++)
    {
        u[i][0] = -2*velocity - u[i][1];
        u[i][ny + 1] = 2*velocity - u[i][ny];
    }
...
void functiondG(double **psi, double **dGpsi)
{
    ...
    ijloop
    {
        // dGpsi[i][j] = 0.5 * (2 * psi[i][j] - 1) * (pow(psi[i][j], 2) - psi[i][j]);
        // printf("psi[%d][%d]%16.12f \n", i, j, psi[i][j]);
